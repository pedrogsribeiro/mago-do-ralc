---
type: index
summary: "Índice Mestre de Navegação do Workspace Mago: A Ascensão 20 Anos (Presencial). Catálogo completo com links diretos para PJs, NPCs, Lugares, Facções, Regras, Umbra e Sessões."
tags: [index, navegação, resumo, rpg, mago, m20]
---

# ÍNDICE MESTRE — Mago: A Ascensão 20 Anos (Presencial)

Este documento é o **ponto de entrada primário para agentes de IA** e jogadores navegarem por todo o acervo da campanha. Ele permite que qualquer entidade, local, regra ou sessão seja encontrada em 1 única leitura.

---

## 👥 1. Personagens dos Jogadores (PJs)

| Nome | Conceito / Fiação | Fichas / Nota |
| :--- | :--- | :--- |
| **Andrey Golubenko** | Cultivador e Assassino | [`Andrey_Golubenko.md`](07_pjs/andrey_golubenko.md) |
| **Hughie** | Ex-militar cirurgião competente e sarcástico | [`Hughie.md`](07_pjs/hughie.md) |
| **Jhonny D. Lee** | Ator Marcial Ascendente | [`Jhonny_D_Lee.md`](07_pjs/jhonny_d_lee.md) |
| **Niki** | Criança prodígio de 11 anos | [`Niki.md`](07_pjs/niki.md) |

---

## 🎭 2. Personagens Não-Jogadores (NPCs)

| Nome | Tipo | Conceito Diegético | Nota do NPC |
| :--- | :--- | :--- | :--- |
| **Ângela** | Desperta | Enfermeira do CAPS dedicada ao tratamento de vítimas de drogas sintéticas | [`Angela.md`](02_npcs/angela.md) |
| **Bernardino** | Adormecido | Influenciador antivax, ex-BBB e herdeiro tradicional | [`Bernardino.md`](02_npcs/bernardino.md) |
| **Carlito Heizenberg** | Desperto | Alquimista criador de droga sintética feita a partir de sangue residual | [`Carlito_Heizenberg.md`](02_npcs/carlito_heizenberg.md) |
| **Elizabeth Barcelos** | Adormecida | Pesquisadora infectologista da vacina da pandemia de 2019-2022 | [`Elizabeth_Barcelos.md`](02_npcs/elizabeth_barcelos.md) |
| **Joana Pipoquinha** | Espírito | Espírito da filha falecida de Lizander que odeia mágika | [`Joana_Pipoquinha.md`](02_npcs/joana_pipoquinha.md) |
| **Lizander (Filho do Raio)** | Desperto | Mestre de Forças extremista usando trauma da pandemia para alimentar Quintessência | [`Lizander_Filho_do_Raio.md`](02_npcs/lizander_filho_do_raio.md) |

---

## 📍 3. Lugares e Cenários

| Local | Descrição Diegética | Arquivo |
| :--- | :--- | :--- |
| **Fábrica Pasta da Nona** | Instalação industrial abandonada reutilizada como laboratório ou refúgio | [`Fabrica_Pasta_da_Nona.md`](03_lugares/fabrica_pasta_da_nona.md) |
| **Sets de Filmagem SP** | Estúdios e sets urbanos de São Paulo usados pela mídia e entretenimento | [`Sao_Paulo_Sets_de_Filmagem.md`](03_lugares/sao_paulo_sets_de_filmagem.md) |
| **Sede Data Center** | Centro de dados corporativo de alta segurança sob vigilância da Tecnocracia/ABIN | [`Sede_Data_Center.md`](03_lugares/sede_data_center.md) |

---

## 🏛️ 4. Facções e Organizações

| Facção | Descrição / Filosofia | Arquivo |
| :--- | :--- | :--- |
| **ABIN** | Agência Brasileira de Inteligência e monitoramento estatal de anomalias | [`ABIN.md`](05_faccoes/abin.md) |
| **Adormecidos** | População profana responsável pela sustentação do Consenso | [`Adormecidos.md`](05_faccoes/adormecidos.md) |
| **Espíritos das Cidades** | Entidades urbanas e efêmeras habitando a Penumbra das metrópoles | [`Espiritos_das_Cidades.md`](05_faccoes/espiritos_das_cidades.md) |
| **Independentes** | Magos e operativos sem afiliação formal às Tradições ou Tecnocracia | [`Independentes.md`](05_faccoes/independentes.md) |
| **Irmãos de Akasha** | Tradição mística focada na Mente, Do e harmonia corpo-espírito | [`Irmaos_de_Akasha.md`](05_faccoes/irmaos_de_akasha.md) |
| **Órfãos** | Despertos autodidatas sem treinamento de uma Tradição estabelecida | [`Orfaos.md`](05_faccoes/orfaos.md) |
| **Pastafarianos** | Grupo satírico/místico manipulador do Paradoxo e crença popular | [`Pastafarianos.md`](05_faccoes/pastafarianos.md) |
| **Povos da Floresta** | Xamãs e guardiões da Umbra Profunda e espíritos da natureza | [`Povos_da_Floresta.md`](05_faccoes/povos_da_floresta.md) |
| **Tecnocracia** | União Tecnocrática dedicada ao controle do Consenso e erradicação do misticismo | [`Tecnocracia.md`](05_faccoes/tecnocracia.md) |

---

## ⚡ 5. Arcos Narrativos (Problemas)

| Arco | Foco Narrativo | Arquivo |
| :--- | :--- | :--- |
| **Problema Legado** | Herança histórica e desdobramentos da pandemia e vacinas | [`Problema_Legado.md`](01_problemas/problema_legado.md) |
| **Problema Presente** | Conflito ativo com a droga sintética refinada e alquimia de sangue | [`Problema_Presente.md`](01_problemas/problema_presente.md) |
| **Problema Futuro** | Ameaça iminente de Lizander e destruição espiritual/tecnocrática | [`Problema_Futuro.md`](01_problemas/problema_futuro.md) |

---

## 🎲 6. Registro de Sessões

| Sessão | Conteúdo / Resumo | Arquivo |
| :--- | :--- | :--- |
| **Sessão 00** | Criação de personagens, alinhamento de expectativas e premissa | [`Sessao_00.md`](04_sessoes/sessao_00.md) |
| **Sessão 01** | Framework de introdução e abertura dos arcos dramáticos | [`Sessao_01_Framework.md`](04_sessoes/sessao_01_framework.md) |

---

## 🎒 7. Itens e Parafernália Especial

| Item | Descrição / Uso Mecânico | Arquivo |
| :--- | :--- | :--- |
| **Cristais de Quintessência** | Matéria condensada de Tass para abastecimento de Arete/Feitiços | [`Cristais_de_Quintessencia.md`](06_itens/cristais_de_quintessencia.md) |
| **Mentes dos Despertos** | Registros e impressões psíquicas de magos | [`Mentes_dos_Despertos.md`](06_itens/mentes_dos_despertos.md) |
| **Sangue** | Fluído biológico usado na alquimia da droga de Carlito | [`Sangue.md`](06_itens/sangue.md) |

---

## 📜 8. Bíblia de Regras M20

### 8.1. Esferas de Mágika
- [`Esfera_Correspondencia.md`](00_biblia/05_regras_de_magika/esfera_correspondencia.md)
- [`Esfera_Entropia.md`](00_biblia/05_regras_de_magika/esfera_entropia.md)
- [`Esfera_Espirito.md`](00_biblia/05_regras_de_magika/esfera_espirito.md)
- [`Esfera_Forcas.md`](00_biblia/05_regras_de_magika/esfera_forcas.md)
- [`Esfera_Materia.md`](00_biblia/05_regras_de_magika/esfera_materia.md)
- [`Esfera_Mente.md`](00_biblia/05_regras_de_magika/esfera_mente.md)
- [`Esfera_Primordio.md`](00_biblia/05_regras_de_magika/esfera_primordio.md)
- [`Esfera_Tempo.md`](00_biblia/05_regras_de_magika/esfera_tempo.md)
- [`Esfera_Vida.md`](00_biblia/05_regras_de_magika/esfera_vida.md)

### 8.2. Regras de Mágika, Paradoxo e Ajustes
- [`Regras_Magika_Passo_a_Passo.md`](00_biblia/05_regras_de_magika/regras_magika_passo_a_passo.md)
- [`Regras_Magika_Paradoxo_e_Reacao.md`](00_biblia/05_regras_de_magika/regras_magika_paradoxo_e_reacao.md)
- [`Regras_Ajustes_Procedimentos_e_Rotinas.md`](00_biblia/05_regras_de_magika/regras_ajustes_procedimentos_e_rotinas.md)
- [`Regras_Magika_Modificadores_Dificuldade.md`](00_biblia/05_regras_de_magika/regras_magika_modificadores_dificuldade.md)
- [`Regras_Magika_Dano_Cura_Duracao.md`](00_biblia/05_regras_de_magika/regras_magika_dano_cura_duracao.md)
- [`Regras_Magika_Quintessencia_e_Ressonancia.md`](00_biblia/05_regras_de_magika/regras_magika_quintessencia_e_ressonancia.md)
- [`Regras_Zonas_de_Realidade_e_Consenso.md`](00_biblia/05_regras_de_magika/regras_zonas_de_realidade_e_consenso.md)
- [`Regras_Focos_e_Instrumentos.md`](00_biblia/05_regras_de_magika/regras_focos_e_instrumentos.md)

### 8.3. Umbra, Película e Espíritos
- [`Umbra.md`](00_biblia/resumos/umbra/umbra.md)
- [`Umbra_Camadas_e_Vidare.md`](00_biblia/resumos/umbra/umbra_camadas_e_vidare.md)
- [`Umbra_Combate_e_Espiritos.md`](00_biblia/resumos/umbra/umbra_combate_e_espiritos.md)
- [`Umbra_Navegacao_e_Rotas.md`](00_biblia/resumos/umbra/umbra_navegacao_e_rotas.md)
- [`Umbra_Travessia_e_Pelicula.md`](00_biblia/resumos/umbra/umbra_travessia_e_pelicula.md)

---

## 📐 9. Estudos de Game Design (Sistema Player-Faced Total)

| Estudo | Tópico e Foco Mecânico | Arquivo |
| :--- | :--- | :--- |
| **Estudo 00** | Manifesto: Problemática, Estimativas de Tempo e Ergonomia Cognitiva | [`00_Manifesto_Ergonomia_e_Carga_Cognitiva.md`](08_estudos_player_faced/00_manifesto_ergonomia_e_carga_cognitiva.md) |
| **Estudo 01** | Rolagens de Obstáculo, 4 Faixas de Resolução e Matemática d10 | [`01_Testes_Obstaculo_e_Matematica_Base.md`](08_estudos_player_faced/01_testes_obstaculo_e_matematica_base.md) |
| **Estudo 02** | Oposição Ativa, Matriz de Conversão Ficha $\rightarrow$ Fator de Ameaça e Relógios | [`02_Oposicao_Ativa_e_Conversao_de_Fichas.md`](08_estudos_player_faced/02_oposicao_ativa_e_conversao_de_fichas.md) |
| **Estudo 03** | Nível 3: Ameaça Telegrafada, Decisões Táticas e Provas Matemáticas | [`03_Chefes_e_Movimentos_Telegrafados.md`](08_estudos_player_faced/03_chefes_e_movimentos_telegrafados.md) |
| **Estudo 04** | Taxonomia dos 3 Níveis e Nível 1 (Desafio Rápido e Oposição Pontual) | [`04_Taxonomia_e_Oposicao_Rapida.md`](08_estudos_player_faced/04_taxonomia_e_oposicao_rapida.md) |
| **Estudo 05** | Artigo Técnico: Matemática, Algoritmos Python e Provas Monte Carlo | [`05_Artigo_Matematica_e_Algoritmos_Player_Faced.md`](08_estudos_player_faced/05_artigo_matematica_e_algoritmos_player_faced.md) |
| **Estudo 06** | Nível 2 — Ameaça Ativa e a Simetria das 4 Características | [`06_Nivel_3_Ameaca_Ativa_e_Simetria.md`](08_estudos_player_faced/06_nivel_3_ameaca_ativa_e_simetria.md) |
| **Estudo 07** | Adaptação de Entidades Espirituais (Umbróides) e a Umbra nos 3 Níveis | [`07_Entidades_Espirituais_e_Umbroides.md`](08_estudos_player_faced/07_entidades_espirituais_e_umbroides.md) |
| **Estudo 08** | Simulação Prática de Mesa (Macro-Crônica e Níveis 1 a 3) | [`08_Simulacao_Pratica_da_Mesa.md`](08_estudos_player_faced/08_simulacao_pratica_da_mesa.md) |
| **Estudo 09** | Guia Prático de Conversão de Fichas de NPCs (Mortais, Criaturas e Espíritos) | [`09_Guia_Pratico_Conversao_de_Fichas_NPC.md`](08_estudos_player_faced/09_guia_pratico_conversao_de_fichas_npc.md) |
| **Estudo 10** | Ontologia Completa e Catálogo de Obstáculos de Mago sob a Regra de Bronze | [`10_Ontologia_e_Catalogo_de_Obstaculos_de_Mago.md`](08_estudos_player_faced/10_ontologia_e_catalogo_de_obstaculos_de_mago.md) |
| **Estudo 11** | Domínio 1: Antagonistas Conscientes nos 3 Níveis (Tecnocracia, Nefandi, etc.) | [`11_Dominio_1_Antagonistas_Conscientes.md`](08_estudos_player_faced/11_dominio_1_antagonistas_conscientes.md) |
| **Estudo 12** | Domínio 2: Constructos e Hipertecnologia nos 3 Níveis (Hit-Marks, Golens, etc.) | [`12_Dominio_2_Constructos_e_Hipertecnologia.md`](08_estudos_player_faced/12_dominio_2_constructos_e_hipertecnologia.md) |

---

## ⚡ 10. Arco 01 da Crônica: O Sangue da Metrópole (The Alexandrian Node Graph)

| Nó Investigativo | Tópico e Foco em Cena | Arquivo |
| :--- | :--- | :--- |
| **Visão Geral** | Índice e Matriz Alexandrian das 3 Pistas Mestre | [`00_visao_geral_e_matriz_de_pistas.md`](09_arco_01_o_sangue_da_metropole/00_visao_geral_e_matriz_de_pistas.md) |
| **Nó A (Set)** | O Incidente Inicial: Explosão do Delivery de Sangue | [`01_no_a_set_de_filmagem.md`](09_arco_01_o_sangue_da_metropole/01_no_a_set_de_filmagem.md) |
| **Nó B (Fábrica)** | A Refinaria Alquímica de Carlito Heizenberg & Pastafarianos | [`02_no_b_fabrica_pasta_da_nona.md`](09_arco_01_o_sangue_da_metropole/02_no_b_fabrica_pasta_da_nona.md) |
| **Nó C (CAPS)** | O Surto de Viciados, Enfermeira Ângela e Joana Pipoquinha | [`03_no_c_caps_zona_sul.md`](09_arco_01_o_sangue_da_metropole/03_no_c_caps_zona_sul.md) |
| **Nó D (Data Center)**| Infiltração na ABIN, Lives de Bernardino e Vacinas | [`04_no_d_data_center_e_abin.md`](09_arco_01_o_sangue_da_metropole/04_no_d_data_center_e_abin.md) |
| **Nó E (Santuário)** | O Clímax (Nível 3 Chefe): O Ritual de Raios de Lizander | [`05_no_e_santuario_de_lizander.md`](09_arco_01_o_sangue_da_metropole/05_no_e_santuario_de_lizander.md) |








